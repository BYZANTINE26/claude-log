...
### Added
- `/claude-log-load [count] --compiled` — consolidates the recent
  entries' summaries into one narrative via the configured
  summarization endpoint, instead of printing them line-by-line. Falls
  back to the normal numbered list if no endpoint is configured or the
  compile call fails, never an error and never nothing.

### Changed
- `claude_log/summarizer.py`'s `call_openai_compatible_endpoint`/
  `call_claude_code_provider` now take a generic `(system_prompt,
  user_content)` instead of turn-summarization-specific arguments, so
  the new `compile_summaries()` reuses the same HTTP/subprocess
  plumbing and never-fabricate-on-failure contract as `summarize()`.
